...
onready var neck_attachment = $Mesh/Godot_Chan_Stealth_Shooter/Godot_Chan_Stealth/Skeleton/neck_attachment # muzzle flash
...
var splatter = 0
var weapon_ray_tip = Vector3() #weapon ray
...
func _input(event):
	...
	if weapon_blend_target:
		...
		if event.is_action_pressed("reload"):# reload mechanics
			reload()# reload mechanics
	...
	if !radial_menu:
		...
		if (Input.is_action_pressed("aim") || Input.is_action_pressed("fire") || !$aim_stay_delay.is_stopped()) && !$AnimationTree.get(roll_active) && weapon_blend_target == 1:
			...
			if Input.is_action_pressed("fire"):
				...
				if $shoot_timer.is_stopped() && $reload_timer.is_stopped() && !$AnimationTree.get(weapon_switch_active) && $WeaponStats.mag() > 0 && ($WeaponStats.auto() || !fired_once):# weapon stats # weapon switch anim

					fired_once = true# weapon stats
					
					$shoot_timer.start(1 / $WeaponStats.fire_rate())
					$shoot_sfx.stream.audio_stream = weapon_fire[current_weapon]
					$shoot_sfx.play()
				
					$WeaponStats.mag_decrement()# weapon stats # decrement 1 bullet
				
					$UI/Crosshair.fire($WeaponStats.fire_rate() * 0.2)# crosshair
				
					neck_attachment.get_node("AnimationPlayer").playback_speed = clamp($WeaponStats.fire_rate(), 5, 10)# muzzle flash
					neck_attachment.get_node("AnimationPlayer").play("muzzle_flash")# muzzle flash
					...
					var spread = $UI/Crosshair.pos_x/12# recoil mechanics
					weapon_ray.rotation_degrees.x = rand_range(-spread, spread)# recoil mechanics
					weapon_ray.rotation_degrees.y = rand_range(-spread, spread)# recoil mechanics
					
					$Camroot/h/v.rotate_x(deg2rad($WeaponStats.recoil()))# recoil mechanics
					$Camroot.recoil_recovery()# recoil mechanics
		...
		if $WeaponStats.mag() < 1 && $reload_timer.is_stopped() && !$AnimationTree.get(roll_active): # reload mechanics
			reload() # reload mechanics
		
		aim_turn = 0
		
		$UI/Crosshair.pos_x = $WeaponStats.spread() + $WeaponStats.movement_spread() * velocity.length() + $WeaponStats.jump_spread() * int(!is_on_floor())# crosshair
		$UI/Crosshair.pos_x += $WeaponStats.aim_spread() * $CamAnimTree.get(cam_aim) + $WeaponStats.crouch_spread() * (1 - crouch_stand_target)# crosshair
		
		$UI/Mag/mag.text = String($WeaponStats.mag())# ui mag
		$UI/Mag/ammo_backup.text = String($WeaponStats.ammo_backup())# ui mag
...
func roll():
	...
	$AnimationTree.set(reload_active, false) # reload mechanics #cancelling reload if rolling
	$reload_timer.stop() # reload mechanics #cancelling reload if rolling
...
func switch_weapon(to):
	...
	if to < weapons.size() && (to != current_weapon || weapon_blend_target == 0):# bugfix
		weapon_blend_target = 1# bugfix
		gun_attachment.show()# bugfix
		...
		$AnimationTree.set("parameters/weapon_change_switch/blend_position", to)# weapon switch anim
		...
		$AnimationTree.set("parameters/weapon_switch_scale/scale", $WeaponStats.switch_speed())# weapon switch anim
		$AnimationTree.set("parameters/weapon_switch_seek/seek_position", 0)# weapon switch anim
		$AnimationTree.set(weapon_switch_active, true)# weapon switch anim
		
		$reload_timer.stop()# reload mechanics #cancelling reload if weapon switching
		$AnimationTree.set(reload_active, false)# reload mechanics #cancelling reload if weapon switching
		
	
		neck_attachment.get_node("muzzle_flash").speed_scale = $WeaponStats.fire_rate()# muzzle flash
		neck_attachment.get_node("streaks").speed_scale = $WeaponStats.fire_rate() / 1.45# muzzle flash
		neck_attachment.get_node("streaks").process_material.gravity.z = -(8000 / $WeaponStats.fire_rate())# muzzle flash


func reload():# reload mechanics
...
func _on_reload_timer_timeout():# reload mechanics
