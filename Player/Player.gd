...
var movement_walk_speed = 2.2
var crouch_walk_speed = 1
var movement_run_speed = 5
var acceleration = 6
var angular_acceleration = 7
var movement_sprint_toggle = false
var sprinting = false
...
func _ready():
	...
	if(!editor):	
		...
		$AnimationTree.set("parameters/walk_scale/scale", movement_walk_speed)
		switch_weapon(0)
		...
		if !radial_menu:
			...
			if Input.is_action_pressed("forward") ||  Input.is_action_pressed("backward") ||  Input.is_action_pressed("left") ||  Input.is_action_pressed("right"):
				...
				if sprinting && $AnimationTree.get(aim_transition) == 1 && crouch_stand_target:
					movement_speed = movement_run_speed
				else:
					if crouch_stand_target:
						movement_speed = movement_walk_speed
					else:
	...
	# ======================================= AIM MODE START ==============================
		...
		if !radial_menu:
			...
			if (Input.is_action_pressed("aim") || Input.is_action_pressed("fire") || !$aim_stay_delay.is_stopped()) && !$AnimationTree.get(roll_active) && weapon_blend_target == 1:
				...
				var iw_blend = (velocity.length() - movement_walk_speed) / movement_walk_speed
				var wr_blend = (velocity.length() - movement_walk_speed) / (movement_run_speed - movement_walk_speed)
			
				#find the graph here: https://www.desmos.com/calculator/4z9devx1ky
			
				if velocity.length() <= movement_walk_speed:
			...
			if is_on_floor():
				...
				if !$AnimationTree.get(roll_active):
					if Input.is_action_just_pressed("jump"):
						...
						vertical_velocity = jr_jump_magnitude
				...
				$AnimationTree.set(jump_blend, lerp($AnimationTree.get(jump_blend), vertical_velocity/jr_jump_magnitude, delta * 10))
...
func roll():
	if(!editor):	
		...
		$roll_timer.start()
		velocity = (direction - get_floor_normal()) * jr_roll_magnitude
...
func _get_property_list():
	var props = []
	add_category("Variables", props)
	add_group("Movement", "movement_" , props)
	add_property("movement_walk_speed", TYPE_REAL, 2.2, props)
	add_property("movement_run_speed", TYPE_REAL, 5, props)
	add_property("movement_sprint_toggle", TYPE_BOOL, true, props)	
	add_group("Jump/Roll", "jr_", props)
	add_property("jr_jump_magnitude", TYPE_REAL, 10, props)
	add_property("jr_roll_magnitude", TYPE_REAL, 20, props)
	add_group("Gravity", "", props)
	add_property("gravity", TYPE_REAL, 28, props)
